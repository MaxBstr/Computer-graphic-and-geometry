...
/*
void Image::NoDithering(uchar BitRate)
{
    ...
    for (int i = 0; i < Height; i++)
    {
        for (int j = 0; j < Width; j++)
        {
            double Value = SRGB_TO_LINEAR(GetPixel(i, j)) / 255.0;

            Value = min(max(Value, 0.0), 1.0);

            double NewColor = round(Value * MaxValue);
            GetPixel(i, j) = LINEAR_TO_SRGB(NewColor / MaxValue * 255);
        }
    }
}

void Image::OrderedDithering(uchar BitRate)
{
    uchar MaxValue = pow(2,BitRate) - 1;
    ...
    for (int i = 0; i < Height; i++)
    {
        for (int j = 0; j < Width; j++)
        {
            double PicColorSRGB = GetPixel(i, j) / 255.0;
            double PicColorLinear = SRGB_TO_LINEAR_ValueCorrection(PicColorSRGB);
            double Value = PicColorLinear + (MatrixOrdered[i % 8][j % 8] - 0.5) / BitRate;
...
void Image::RandomDithering(uchar BitRate)
{
    uchar MaxValue = pow(2, BitRate) - 1;
...
void Image::FloydSteinbergDithering(uchar BitRate)
{
    uchar MaxValue = pow(2, BitRate) - 1;

    vector<double> Errors(Height * Width, 0);
    auto GetError = [&](int h, int w) -> double& { return Errors[h * Width + w]; };
    ...
    for (int i = 0; i < Height; i++)
    {
        for (int j = 0; j < Width; j++)
        {
            ...
            double Error = GetPixel(i, j) + GetError(i, j) - NewColor / (double) MaxValue * 255.0;
...
void Image::JJNDithering(uchar BitRate)
{
    uchar MaxValue = pow(2, BitRate) - 1;
...
void Image::SierraDithering(uchar BitRate)
{
    uchar MaxValue = pow(2, BitRate) - 1;
...
void Image::AtkinsonDithering(uchar bitRate)
{
    uchar MaxValue = pow(2, bitRate) - 1;
...
void Image::HalftoneDithering(uchar BitRate)
{
    uchar MaxValue = pow(2, BitRate) - 1;
    ...
    for (int i = 0; i < Height; i++)
    {
        for (int j = 0; j < Width; j++)
        {
            double PicColorSRGB = GetPixel(i, j) / 255.0;
            double PicColorLinear = SRGB_TO_LINEAR_ValueCorrection(PicColorSRGB);
            double Value = PicColorLinear + (MatrixHalftone[i % 4][j % 4] - 0.75) / BitRate;
            Value = min(max(Value, 0.0), 1.0);

            double NewColor = round(Value * MaxValue);

            GetPixel(i, j) = round(NewColor / MaxValue * 255);
        }
    }
}*/
...
/*
uchar Image::SRGB_TO_LINEAR(uchar Color)
{
    double ColorSRGB = Color / 255.0;
    double ColorLinear = SRGB_TO_LINEAR_ValueCorrection(ColorSRGB);
    return ColorLinear * 255;
}

double Image::SRGB_TO_LINEAR_ValueCorrection(double Value)
{
    return Value <= 0.04045 ? Value / 12.92 : pow((Value + 0.055) / 1.055, 2.4);
}

uchar Image::LINEAR_TO_SRGB(uchar Color)
{
    double ColorLinear = Color / 255.0;
    double ColorSRGB = LINEAR_TO_SRGB_ValueCorrection(ColorLinear);
    return ColorSRGB * 255;
}

double Image::LINEAR_TO_SRGB_ValueCorrection(double Value)
{
